...
#include "commands/z.h"
#include "assert.h"

compress::compress(const Glow::Util::file &archive, unsigned short compression_method, int level)
: archive_(archive),compression_method_(compression_method), level_(level) {
  if (level < Glow::Command::COMPRESSION_LEVEL_0 || level > Glow::Command::COMPRESSION_LEVEL_9) {
    THROW(Glow::Command::UnsupportedCompressionLevelException,"Compression level %u not supported",level);
  }
}

unsigned long compress::writing_position() {
...
unsigned short compress::LevelToZipFlag() {
...
central_directory_header compress::header_from_file(Glow::Util::file &fileToAdd, const Glow::Util::file &base_dir) {
  ...
  std::string a_la_zip_path=Glow::Util::file(relative_path).get_unix_path();
...
local_file_header compress::central_to_local(const central_directory_header& cdh) {
...
unsigned long compress::write_local_file_header(const local_file_header& lfh) {
...
central_directory_header compress::compress_and_write_data (Glow::Command::compressor &comp, const central_directory_header& cdh) {
...
void compress::write_central_directory() {
...
void compress::add_file(Glow::Util::file &fileToAdd, const Glow::Util::file &base_dir) {
  ...
  if (fileToAdd.is_directory()) {
    Glow::Util::fs f(fileToAdd);
    std::vector<Glow::Util::file> files = f.content();
    std::vector<Glow::Util::file>::iterator it;
...
void compress::deflate() {
  ...
  for (it=catalog.begin();it != catalog.end();it++) {
    ...
    if (!empty_it) {
      ...
      Glow::Command::Z zipator(to_zip_stream,archive_stream_,level_);
