...
uncompress::uncompress(Glow::Util::file & archive) : archive_(archive) {
...
void uncompress::inflate() {
  Glow::Util::file to(".");
  inflate(to);
}

void uncompress::inflate(Glow::Util::file & to) {
  for(int i = 0; i < number_of_entries(); i++) {
    inflate_file_at_index(i, to);
  }
}

int uncompress::number_of_entries() {
  return cdh_.size();
}

void uncompress::read_eocdh() {
...
void uncompress::inflate_file_at_index(int i) {
  Glow::Util::file to(".");
  inflate_file_at_index(i, to);
}

void uncompress::inflate_file_at_index(int i, Glow::Util::file & to) {
  ...
  switch(e.compression_method) {
    case CM_STORE:
      {
        Glow::Util::file outfile = to.join(e.file_name);
        if(outfile.represents_directory()) {
          ...
          Logger::info("Uncompress file %s", outfile.get_path().c_str());
    ...
    case CM_DEFLAT:
      {
        ...
        Glow::Command::compressor * cmp = new Glow::Command::Z(os, is_);
        cmp->decompress(e.compressed_size);
...
local_file_header uncompress::read_lfh_at_index(int i) {
...
entry uncompress::file_stat_at_index(int i) {
  entry e;
  ...
  while(pos < cdh.data_part.extra_field.end()) {
    ...
    switch(header_id) {
      ...
      case EF_UNIX:
        ...
        break;
      default:
        Logger::warning("Unsupported extra field 0x%x", header_id);
    }
